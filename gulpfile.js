...
gulp.task('server', ['sass', 'babel'], () => {
  ...
  watch(dirs.es6 + "/*.js")
    ...
    .on('unlink', function (file) {
      ...
      fileSystem.existsSync(dirs.js + '/' + _file) && fileSystem.unlink(dirs.js + '/' + _file);
      fileSystem.existsSync(dirs.js + '/' + _file + '.map') && fileSystem.unlink(dirs.js + '/' + _file + '.map');
  ...
  watch(dirs.sass + "/*.scss")
    ...
    .on('unlink', function (file) {
      ...
      fileSystem.existsSync(dirs.css + '/' + _file) && fileSystem.unlink(dirs.css + '/' + _file);
      fileSystem.existsSync(dirs.css + '/' + _file + '.map') && fileSystem.unlink(dirs.css + '/' + _file + '.map');
      console.log('删除文件:' + dirs.css + '/' + _file)
    })

  watch(dirs.sassCom + "/*.scss")
    .on('add', function () {
